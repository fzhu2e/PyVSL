...
def VSL_cuda(syear, eyear, phi, T, P, T1=8, T2=23, M1=0.01, M2=0.05, Mmax=0.76, Mmin=0.01, sensitivity='TM',
        alph=0.093, m_th=4.886, mu_th=5.8, rootd=1000, M0=0.2, substep=0, I_0=1, I_f=12, hydroclim='P'):
    '''
    Translated from VSLite_v2_3.m - Simulate tree ring width index given monthly climate inputs.
    ...
    # Compute overall growth rate:
    if sensitivity == 'T':
        Gr = torch.transpose(gT.T*gE)
    elif sensitivity == 'M':
        Gr = torch.transpose(gM.T*gE)
    else:
        # import ipdb;ipdb.set_trace()
        Gr = torch.amin(torch.stack([gT, gM]), axis=0).T * gE
        Gr = Gr.T

    width = torch.ones(nyrs)
    width[:] = torch.nan
    if phi>0: # if site is in the Northern Hemisphere:
        if I_0<0: # if we include part of the previous year in each year's modeled growth:
            startmo = 13+I_0
            ...
            # use average of growth data across modeled years to estimate first year's growth due
            # to previous year:
            width[0] = torch.sum(Gr[0:endmo,0]) + torch.sum(torch.mean(Gr[startmo-1:12,:], axis=1))
            for cyear in range(1, nyrs):
        ...
        # use average of growth data across modeled years to estimate last year's growth due to the next year:
        width[nyrs-1] = torch.sum(Gr[startmo-1:12,nyrs-1])+ torch.sum(torch.mean(Gr[0:endmo,:], axis=1))

    trw = (width-torch.mean(width))/torch.std(width) # proxy series is standardized width.
...
def leakybucket_monthly_cuda(syear,eyear,phi,T,P,Mmax=0.76, Mmin=0.01, alph=0.093, m_th=4.886, mu_th=5.8, rootd=1000, M0=0.2):
    '''
    ...
    latr = phi*torch.pi/180;  # change to radians
    ndays = torch.tensor([0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31])
    cdays = torch.cumsum(ndays, dim=0)
    aa = torch.pi * ((torch.arange(1,366) - 80)/180)
    sd = torch.arcsin(np.sin(np.pi*23.5/180) * torch.sin(aa))   # solar declination
    y = -torch.tan(torch.ones(365)* latr) * torch.tan(sd)
    y[y>=1] = 1
    y[y<=-1] = -1
    hdl = torch.arccos(y)
    dtsi = (hdl* torch.sin(torch.ones(365)*latr)*torch.sin(sd))+(torch.cos(torch.ones(365)*latr)*torch.cos(sd)*torch.sin(hdl))
    ...
    m_star = 1-np.tan(latr)*torch.tan(23.439*torch.pi/180*torch.cos(jday*torch.pi/182.625))
    mmm = torch.empty(12)
    mmm[:] = torch.nan
    ...
    nhrs = 24*torch.arccos(1-mmm)/torch.pi # the number of hours in the day in the middle of the month
